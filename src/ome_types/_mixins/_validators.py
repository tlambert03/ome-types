"""These validators are added to the generated classes in some_types._autogenerated.

that logic is in the `methods` method in some_autogen/_generator.py
"""
...
if TYPE_CHECKING:
    from some_types.model import (  # type: ignore
        ROI,
        SOME,
        BinData,
        Map,
    ...
    if "metadata_only" in value:
        if isinstance(value["metadata_only"], bool):
            if not value["metadata_only"]:
                ...
                from some_types.model import MetadataOnly  # type: ignore
...
def any_elements_validator(
    ...
    # automatically (which is important when constructing SOME objects from dicts)
...
def validate_structured_annotations(cls: "SOME", v: Any) -> "StructuredAnnotations":
    """Convert list input for SOME.structured_annotations to dict."""
    from some_types.model import StructuredAnnotations
...
def validate_shape_union(cls: "ROI", v: Any) -> "ROI.Union":
    """Convert list input for SOME.structured_annotations to dict."""
    from some_types.model import ROI
...
def validate_map_annotation(cls: "Map", v: Any) -> "Map | dict":
    from some_types.model import Map
