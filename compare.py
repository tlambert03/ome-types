...
def get_diffs(
    pth1="ome_types_v1.json", pth2="ome_types_v2.json"
) -> tuple[list, list, dict, dict]:
    data1 = json.loads(Path(pth1).read_text())
    data2 = json.loads(Path(pth2).read_text())
...
def _cls_field(key: str) -> tuple[str, str]:
...
GLOBAL_CHANGES = [
    ...
    (
        ...
        "attributes on instances, but will emit a deprecation warning."
    ),
    (
        "`PositiveInt`, `PositiveFloat`, `NonNegativeInt`, and `NonNegativeFloat` "
        "are no longer subclasses of pydantic `ConstrainedInt` and `ConstrainedFloat`,"
        " but are now simply `int` or `float` type with field validators."
    ),
    (
    ...
    (
        "The `kind` fields that were present on many `Shape` or `LightSourceGroup` "
        "subclasses have been from the models. The `kind` key may still be included "
        "in a `dict` when instantiating a subclass, but the name will not be available "
        "on the model. (This was not an OME field to begin with, it was a workaround.)"
    ),
    (
        "Many fields that take an enum and have a default value have had their types "
        "changed from `Optional[EnumType]` to `EnumType`.  For example "
        "`Optional[UnitsLength]` is now `UnitsLength`."
    ),
    (
        "The `model.simple_types` module no longer declares anything and should not "
        "be used (import directly from `model` instead).  Names that were previously "
        "there are still aliased in `simple_types`, but code should be updated."
    ),
]


def markdown_changes(heading_level: int = 2) -> str:
    lines: list[str] = ["# Changes for `ome-types` v0.4.0", ""]
    hd1 = "#" * heading_level
    hd2 = "#" * (heading_level + 1)
    class_changes, module_changes = gather_classes()

    lines.extend(["", f"{hd1} General Changes", ""])
    for change in GLOBAL_CHANGES:
        lines.append("- " + change)

    lines.extend(["", f"{hd1} `ome_types.model` Module Changes", ""])
...
if __name__ == "__main__":
    dump_fields()
    Path("changes.md").write_text(markdown_changes())
